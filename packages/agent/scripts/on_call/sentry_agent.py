...
"""
    ...
    approval_msg = f"🔬 <b>Sentry Diagnosis Ready</b>\n\nShould I open a GitHub issue in <code>{repo_full_name}</code> with this diagnosis?"
    
    import time
    approved = request_approval(
        approval_msg,
        f"sentry_issue_{issue_id}_{int(time.time())}",
        repo_full_name=repo_full_name
    )
    ...
    if approved:
        ...
        try:
            import subprocess
            issue_title = f"Sentry: {title}"
            issue_body = f"## Sentry Diagnosis\n\n{diagnosis}\n\n[View on Sentry]({permalink})\n\n---\n*Created automatically by Hermes*"
