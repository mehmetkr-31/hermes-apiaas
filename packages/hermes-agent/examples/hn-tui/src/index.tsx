...
function App() {
	...
	const handleGenerate = async () => {
		...
		try {
			...
			if (r.ok) {
				...
				setGenLogs((prev) => [
					...prev,
					"[!] Manager rejected request. Is the Manager running?",
				]);
			}
		} catch {
			setGenLogs((prev) => [
				...prev,
				"[!] Cannot reach Manager. Run: python scripts/manager.py",
			]);
		}
	};
	...
	useKeyboard((key) => {
		if (view === "LIST") {
			...
			if (key.name === "down" || key.name === "j")
				setSelectedIndex((i) => Math.min(apis.length - 1, i + 1));
			if (key.name === "n") {
				setView("NEW_API");
				setFormFocus(0);
			}
		...
		if (view === "NEW_API") {
			...
			if ((key.name === "enter" || key.name === "return") && formFocus === 2)
				handleGenerate();
	...
	const Header = (
		...
		>
			<text fg="cyan" attributes={TextAttributes.BOLD}>
				⬡ WEAVER — Agentic Data Service
			</text>
			<box flexDirection="row" gap={2}>
				<text fg="dim">[ </text>
				<text
					fg={view === "LIST" ? "white" : "dim"}
					attributes={
						view === "LIST" ? TextAttributes.BOLD : TextAttributes.NONE
					}
				>
					LIST
				</text>
				<text fg="dim"> | </text>
				<text
					fg={view === "NEW_API" ? "white" : "dim"}
					attributes={
						view === "NEW_API" ? TextAttributes.BOLD : TextAttributes.NONE
					}
				>
					NEW API
				</text>
				<text fg="dim"> | </text>
				<text
					fg={view === "DATA" ? "white" : "dim"}
					attributes={
						view === "DATA" ? TextAttributes.BOLD : TextAttributes.NONE
					}
				>
					DATA
				</text>
			...
			<box flexDirection="row" gap={1}>
				<text fg={managerOnline ? "green" : "red"}>
					{managerOnline ? "●" : "○"}
				</text>
				<text
					fg={managerOnline ? "green" : "red"}
	...
	const renderList = () => (
		<box flexDirection="column" flexGrow={1}>
			<scrollbox
				...
				borderStyle="single"
				borderColor="dim"
				title={` ${apis.length} Active API${apis.length !== 1 ? "s" : ""} `}
				focused
			>
				{apis.length === 0 ? (
					<box
						...
						return (
							...
							>
								...
								<box flexDirection="column" alignItems="flex-end" gap={0}>
									...
									<text fg="dim" attributes={TextAttributes.DIM}>
										:{api.port} {timeAgo(api.created_at)}
									</text>
								</box>
							</box>
						);
			...
			<box height={1} marginTop={1} paddingX={1} justifyContent="space-between">
				<text fg="dim">
					↑↓: Select | ENTER: View Data | D: Delete | N: New API
				</text>
	...
	const renderNewApi = () => (
		<box flexDirection="column" flexGrow={1} gap={1}>
			<box
				...
				border
				borderStyle="single"
				borderColor={formFocus === 0 ? "yellow" : "dim"}
				padding={1}
				title=" Target URL "
			>
				<input
			...
			<box
				...
				border
				borderStyle="single"
				borderColor={formFocus === 1 ? "yellow" : "dim"}
				padding={1}
				title=" Data Schema "
			>
				<textarea
			...
			>
				<text
					fg={formFocus === 2 ? "green" : "dim"}
					attributes={
						formFocus === 2 ? TextAttributes.BOLD : TextAttributes.DIM
					}
				>
			...
			<box height={1} marginTop={1} paddingX={1} justifyContent="center">
				<text fg="dim">
					TAB: Switch fields | ENTER on ▶ button: Generate | ESC: Back
				</text>
			</box>
		</box>
	);
	...
	const renderGenerating = () => {
		...
		const hasErr = genLogs.some(
			(l) => l.startsWith("[!]") && !l.includes("Waiting"),
		);

		const phases = [
			{
				label: "⬡ Agent: Launch Browser",
				sub: "Opening real Chromium, navigating to target URL",
				done: has("browser_navigate") || has("BROWSE"),
				active: has("Hermes Agent is opening") && !has("browser_navigate"),
			},
			{
				label: "⬡ Agent: Wait for JS",
				sub: "Waiting for SPA/JS content and lazy-loaded items to fully render",
				done: has("ANALYZE") || has("browser_scroll"),
				active: has("browser_navigate") && !has("ANALYZE"),
			},
			{
				label: "⬡ Agent: Extract Data",
				sub: "Identifying CSS selectors/JSON keys matching your schema",
				done: has("WRITE THE SCRAPER") || has("write_file"),
				active: has("ANALYZE") && !has("write_file"),
			},
			{
				label: "⬡ Agent: Write FastAPI Code",
				sub: "Generating scraper_generated.py with Playwright and CORS",
				done: has("VERIFY") || has("import scraper"),
				active: has("write_file") && !has("VERIFY"),
			},
			{
				label: "Starting Server",
				sub: "Launching uvicorn, polling until first successful HTTP response",
				done: has("API is LIVE"),
				active: has("GENERATION_COMPLETE") && !has("API is LIVE"),
			},
		];
		...
		return (
			<box flexDirection="column" flexGrow={1} gap={1}>
				<box
					...
					border
					borderStyle="double"
					borderColor={hasErr ? "red" : "yellow"}
					padding={1}
					title={hasErr ? " ✗ Generation Failed " : " ⟳ Generation Progress "}
				>
					{phases.map((phase, i) => {
						const color = phase.done
							? "green"
							: phase.active
								? "yellow"
								: "dim";
						const icon = phase.done ? "✓" : phase.active ? "▶" : "○";
						const attr = phase.active
							? TextAttributes.BOLD
							: TextAttributes.NONE;
						return (
						...
						recentLogs.map((log, i) => (
							<text
								...
								fg={
									log.startsWith("[!]")
										? "red"
										: log.startsWith("[✓]") || log.startsWith("[v]")
											? "green"
											: log.startsWith("[+]")
												? "cyan"
												: log.startsWith("[~]")
				...
				<box height={1} paddingX={1}>
					<text fg="dim">
						ESC: Back to List (generation continues in background)
					</text>
				</box>
			</box>
		);
		...
		return (
			<box flexDirection="column" flexGrow={1}>
				<box height={2} paddingX={1} marginBottom={1} flexDirection="column">
					...
					<text fg="dim" attributes={TextAttributes.DIM}>
						Port {api?.port} | Schema: {api?.schema.slice(0, 80)}
					</text>
				</box>
